"""
Variant G — GRU + shared notepad, RL training with cosine reward.

Teacher: frozen gpt-neo-125M (well-trained, 125M params).
Student: GRU + shared notepad, 6 layers, d=1024, GRUCell sequential.

Same cosine reward as Variant F. Notepad updated at every token position —
GRUCell processes sequentially so the write at position t is causally
visible at position t+1 within the same layer.

Compare against Variant F (no notepad, same reward) to isolate notepad value.
"""
import os
import csv
import torch
...
SEED           = 42
EMA_ALPHA      = 0.05
CHECKPOINT_DIR = "checkpoints/variant_g"
LOG_PATH       = "checkpoints/variant_g/loss_log.csv"
...
# ==========================================
# STUDENT — GRU + shared notepad, sequential
# ==========================================
class GRUNotepadBlock(nn.Module):
    ...
    def forward(self, x, note):
        ...
        h_reads = torch.stack(h_reads, dim=1)
        out     = self.ln2(self.mlp(h_reads) + h_reads)
        out     = self.ln_skip(out + x)
        return out, note_vec.unsqueeze(1)

class VariantG(nn.Module):
    def __init__(self):
        super().__init__()
        d = D_MODEL
...
def train():
    ...
    student   = VariantG().to(DEVICE)
    optimizer = optim.AdamW(student.parameters(), lr=LR, foreach=False)
    s_params  = sum(p.numel() for p in student.parameters())
    print(f"Variant G — GRU Notepad Cosine RL | Params: {s_params:,} (~{s_params/1e6:.1f}M)", flush=True)
    ...
    print(f"\n--- VARIANT G | steps {start_step} -> {TOTAL_STEPS} ---", flush=True)
    student.train()
    t0        = time.time()
    step      = start_step
    baseline  = 0.0
