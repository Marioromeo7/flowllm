"""
Variant I — GRU + shared notepad, RL training with teacher log-probability reward.

Teacher: frozen gpt-neo-125M (well-trained, 125M params).
Student: GRU + shared notepad, 6 layers, d=1024, GRUCell sequential.

Same log-prob reward as Variant H. Notepad updated at every token position.

Compare against Variant H (no notepad, same reward) to isolate notepad value.
Compare against Variant G (notepad, cosine reward) to isolate reward signal choice.
"""
import os
import csv
import torch
...
SEED           = 42
EMA_ALPHA      = 0.05
CHECKPOINT_DIR = "checkpoints/variant_i"
LOG_PATH       = "checkpoints/variant_i/loss_log.csv"
...
def train():
    ...
    student   = VariantI().to(DEVICE)
    optimizer = optim.AdamW(student.parameters(), lr=LR, foreach=False)
    s_params  = sum(p.numel() for p in student.parameters())
    print(f"Variant I — GRU Notepad Log-Prob RL | Params: {s_params:,} (~{s_params/1e6:.1f}M)", flush=True)
    ...
    print(f"\n--- VARIANT I | steps {start_step} -> {TOTAL_STEPS} ---", flush=True)
    student.train()
    t0        = time.time()
    step      = start_step
    baseline  = 0.0
