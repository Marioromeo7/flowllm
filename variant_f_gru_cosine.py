"""
Variant F — Pure GRU, RL training with cosine reward.

Teacher: frozen gpt-neo-125M (well-trained, 125M params).
Student: pure GRU, 6 layers, d=1024.

Teacher generates greedy tokens on each input — the target flow.
GRU policy samples its own tokens — the student flow.
Reward at each position = cosine similarity between student's sampled token
and teacher's greedy token, both embedded in gpt-neo's 768-dim token space.

REINFORCE with running mean baseline. No notepad.
Compare against Variant G to isolate notepad value.
"""
import os
import csv
import torch
...
SEED           = 42
EMA_ALPHA      = 0.05
CHECKPOINT_DIR = "checkpoints/variant_f"
LOG_PATH       = "checkpoints/variant_f/loss_log.csv"
...
def collate_fn(batch):
    ...
    return toks.input_ids[:, :-1]  # (B, T) context only
...
def train():
    ...
    teacher = AutoModelForCausalLM.from_pretrained("EleutherAI/gpt-neo-125M", torch_dtype=torch.float16)
    teacher.eval()
    ...
    student   = VariantF().to(DEVICE)
    optimizer = optim.AdamW(student.parameters(), lr=LR, foreach=False)
    s_params  = sum(p.numel() for p in student.parameters())
    print(f"Variant F — GRU Cosine RL | Params: {s_params:,} (~{s_params/1e6:.1f}M)", flush=True)
    ...
    print(f"\n--- VARIANT F | steps {start_step} -> {TOTAL_STEPS} ---", flush=True)
    student.train()
    t0        = time.time()
    step      = start_step
    baseline  = 0.0
    ...
    while step < TOTAL_STEPS:
        ...
        # Student forward — teacher stays on CPU
        ...
        # Teacher reward: move to GPU only for its forward pass
        teacher.cuda()
        with torch.no_grad():
            t_tokens = teacher(x).logits.argmax(dim=-1)             # (B, T)
            t_emb    = teacher.transformer.wte(t_tokens).float()    # (B, T, 768) fp32
            s_emb    = teacher.transformer.wte(s_tokens).float()    # (B, T, 768) fp32
            reward   = F.cosine_similarity(s_emb, t_emb, dim=-1)   # (B, T)
        teacher.cpu()
        torch.cuda.empty_cache()

        # Mask padding, REINFORCE with running baseline
        ...
        optimizer.step()
        loss_val = loss.item()
        del x, s_logits, s_tokens, s_log_probs, t_tokens, t_emb, s_emb, reward, pad_mask, advantage, loss
        step += 1
